...
class Particle {
  ...
  Particle(PVector l, float _r, float vx, float vy, float ax, float ay, boolean isFRun, ParticleSystem ps) {
...
//
//   Confetti(PVector l, float _r, float vx, float vy, float ax, float ay, float isFlowerRun ParticleSystem ps) {
//     super(l, _r, vx, vy, ax, ay, false, ps);
  ...
  void addParticle() {
    if (!isOver){
      ...
      if ( compareMouse() ) {
        ...
        particles.add(new Particle (origin, r, vx, vy, ax, ay, true, self));
      }
      if (!(compareMouse())) {
        vx= 0;
        vy= random(0, 1);
        ax = random(-0.02, 0.04);
        ay = random(0.1);
        particles.add(new Particle (origin, r, vx, vy, ax, ay, false, self));
      }
    }
